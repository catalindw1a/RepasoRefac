...
public class MasterMind extends SuperMasterMind implements InterfazMasterMind {
...
pedirJugada (i);
	...
	public boolean visualizarSalida(boolean acertada, int i, String newParam) {
		System.out.println ();
		if (acertada) {
			quebien = "¡ENHORABUENA! Has acertado la combinación en " + (i-1) +" intentos en la partida nº ";
			System.out.println (quebien +numPartidas);
		} else {
			quepena = "LO SENTIMOS. No has conseguido acertar la combinación en la partida nº ";
	...
	@Override
	public long modificarHeridos(int numRM, int numVM, int numAM, int numBM, int numNM, int numMM, int numGM, int numSM,
